...
static int priv_net_create_socket(int domain, int type, struct sockaddr *addr, int sockaddrlen)
{
	...
	if(sock < 0)
	{
		...
		if(FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM|FORMAT_MESSAGE_IGNORE_INSERTS, 0, error, 0, buf, sizeof(buf), 0) == 0)
			buf[0] = 0;
		printf("failed to create socket with domain %d and type %d (%d '%s')\n", domain, type, error, buf);
#else
		printf("failed to create socket with domain %d and type %d (%d '%s')\n", domain, type, errno, strerror(errno));
#endif
		return -1;
	}
...
int net_udp_recv(NETSOCKET sock, NETADDR *addr, void *data, int maxsize)
{
	...
	if(bytes > 0)
	{
		((struct sockaddr *)&sockaddrbuf)->sa_family = AF_INET;
