		...
		if(!gs_ListBoxDoneEvents)
		{
			...
			else
			{
				for(int i = 0; i < m_NumInputEvents; i++)
				{
					...
					{
						if(m_aInputEvents[i].m_Key == KEY_DOWNARROW) NewIndex = gs_ListBoxNewSelected + 1;
						if(m_aInputEvents[i].m_Key == KEY_UPARROW) NewIndex = gs_ListBoxNewSelected - 1;
					}
...
void CMenus::RenderDemoList(CUIRect MainView)
{
	...
	if(m_DemolistSelectedIndex > -1)
	{
		for(int i = 0; i < m_NumInputEvents; i++)
		{
			...
			if(m_aInputEvents[i].m_Flags&IInput::FLAG_PRESS)
			{
				if(m_aInputEvents[i].m_Key == KEY_DOWNARROW) NewIndex = m_DemolistSelectedIndex + 1;
				if(m_aInputEvents[i].m_Key == KEY_UPARROW) NewIndex = m_DemolistSelectedIndex - 1;
			}
